...
Public Class Form1
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Using g As Graphics = Graphics.FromImage(b)
            ...
            'Dotted Line
            Dim dash() As Single = {widths / 160, widths / 160}
            Dim bp As New Pen(Brushes.Gray, widths / 500)
            bp.DashPattern = dash
            g.DrawRectangle(bp, New Rectangle(New Point(widths \ 10, heights \ 10), New Size(widths * 4 \ 5, heights * 4 \ 5)))
        End Using
    ...
    Sub loadsign(c As Bitmap)
        Using b As Bitmap = c.Clone
            ...
            Dim rect4 As New Rectangle(0, 0, b.Width, b.Height)
            Dim bmpData4 As System.Drawing.Imaging.BitmapData = b.LockBits(rect4, Drawing.Imaging.ImageLockMode.ReadWrite, b.PixelFormat)
            Dim ptr4 As IntPtr = bmpData4.Scan0
            Dim bytes4 As Integer = Math.Abs(bmpData4.Stride) * b.Height
            Dim rgbValues4(bytes4 - 1) As Byte
            Dim pixelbyte4 As Integer = Math.Abs(bmpData4.Stride) \ b.Width 'should be 4
            System.Runtime.InteropServices.Marshal.Copy(ptr4, rgbValues4, 0, bytes4)
            ...
            While s
                For y = 0 To b.Height - 1
                    If rgbValues4((y * b.Width + x) * pixelbyte4 + 3) > 0 Then
                        s = False
                    End If
                Next
                If s Then
            ...
            End While
            bottomempty = y
            System.Runtime.InteropServices.Marshal.Copy(rgbValues4, 0, ptr4, bytes4)
            b.UnlockBits(bmpData4)
            Dim exactsize As New Size(rightempty - leftempty, bottomempty - topempty)
            Dim d As New Bitmap(exactsize.Width * 5 \ 4, exactsize.Height * 5 \ 4)
            Using g As Graphics = Graphics.FromImage(d)
                g.DrawImage(b, New Rectangle(New Point(d.Width \ 10 - leftempty, d.Height \ 10 - topempty), New Size(b.Width, b.Height)))
            End Using
            Using g As Graphics = Graphics.FromImage(signbase)
                Dim aspect As Double = d.Width / d.Height
                Dim offset As Point
                If aspect > 16 / 9 Then 'Too Wide
                    offset = New Point(0, (heights - widths / aspect) \ 2)
                    g.DrawImage(d, New Rectangle(offset, New Size(widths, widths / aspect)))
                Else
                    offset = New Point((widths - aspect * heights) \ 2, 0)
                    g.DrawImage(d, New Rectangle(offset, New Size(heights * aspect, heights)))
                End If
            End Using
        End Using
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        Using g As Graphics = Graphics.FromImage(image)
            ...
            End If
        End Using
        loadsign(My.Resources.Shizuku_Sign2)
    ...
    Private Sub Button6_Click(sender As Object, e As EventArgs) Handles Button6.Click
        If FolderBrowserDialog1.ShowDialog = Windows.Forms.DialogResult.OK Then
            exportprepare()
            Dim fps As Integer = CInt(InputBox("FPS?", "FPS", 30))
            Dim froms As Integer = CInt(InputBox("From Frame?", "Frame", 0))
            Dim count As Integer = froms
            Dim tp As Double = count * 1000 / fps
            While CInt(tp) <= 12200
                Dim f As Integer = Math.Min(CInt(Int(tp * 0.03)), 365) + 1
                Dim base As Bitmap = resizeimg(New Bitmap(Application.StartupPath & "\files\scene" & f.ToString.PadLeft(5, "0") & ".png"))
                If tp >= 4500 And tp <= 7100 Then
                ...
                End If
                'To 12200
                base.Save(FolderBrowserDialog1.SelectedPath & "\scene" & count.ToString.PadLeft(5, "0") & ".png")
                base.Dispose()
                GC.Collect()
                count += 1
                tp = count * 1000 / fps
    ...
    Private Sub Button7_Click(sender As Object, e As EventArgs) Handles Button7.Click
        If SaveFileDialog1.ShowDialog = Windows.Forms.DialogResult.OK Then
            exportprepare()
            Dim fps As Integer = CInt(InputBox("FPS?", "FPS", 30))
            Dim froms As Integer = CInt(InputBox("Frame?", "Frame", 0))
            Dim count As Integer = froms
            Dim tp As Double = count * 1000 / fps
            Dim f As Integer = Math.Min(CInt(Int(tp * 0.03)), 365) + 1
            Dim base As Bitmap = resizeimg(New Bitmap(Application.StartupPath & "\files\scene" & f.ToString.PadLeft(5, "0") & ".png"))
            ...
            End If
            'To 12200
            base.Save(SaveFileDialog1.FileName)
            GC.Collect()
            base.Dispose()
        End If
    End Sub
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        If SaveFileDialog1.ShowDialog = DialogResult.OK Then
            completed.Save(SaveFileDialog1.FileName)
        End If
    End Sub

    Private Sub Button9_Click(sender As Object, e As EventArgs) Handles Button9.Click
        If OpenFileDialog2.ShowDialog = DialogResult.OK Then
            Dim b2 As New Bitmap(OpenFileDialog2.FileName)
            Dim d As New Bitmap(b2.Width + 12, b2.Height + 12)
            Using g As Graphics = Graphics.FromImage(d)
                g.DrawImage(b2, New Rectangle(New Point(6, 6), b2.Size))
            End Using
            Dim border2 As Integer = CInt(Int(Val(InputBox("Border Width?", "Direct Export", 6))))
            Dim rect As New Rectangle(0, 0, d.Width, d.Height)
            Dim bmpData As System.Drawing.Imaging.BitmapData = d.LockBits(rect, Drawing.Imaging.ImageLockMode.ReadWrite, d.PixelFormat)
            Dim ptr As IntPtr = bmpData.Scan0
            Dim bytes As Integer = Math.Abs(bmpData.Stride) * d.Height
            Dim rgbValues(bytes - 1) As Byte
            Dim pixelbyte As Integer = Math.Abs(bmpData.Stride) \ d.Width 'should be 4
            System.Runtime.InteropServices.Marshal.Copy(ptr, rgbValues, 0, bytes)
            For y = 0 To d.Height - 1
                For x = 0 To d.Width - 1
                    If rgbValues((y * d.Width + x) * 4 + 3) > 128 Then
                        rgbValues((y * d.Width + x) * 4 + 3) = 255
                        rgbValues((y * d.Width + x) * 4 + 2) = 255
                        rgbValues((y * d.Width + x) * 4 + 1) = 255
                        rgbValues((y * d.Width + x) * 4 + 0) = 255
                    Else
                        rgbValues((y * d.Width + x) * 4 + 3) = 0
                    End If
                Next
            Next
            System.Runtime.InteropServices.Marshal.Copy(rgbValues, 0, ptr, bytes)
            d.UnlockBits(bmpData)
            Dim d2 As Bitmap = d.Clone
            Dim rect2 As New Rectangle(0, 0, d2.Width, d2.Height)
            Dim bmpData2 As System.Drawing.Imaging.BitmapData = d2.LockBits(rect2, Drawing.Imaging.ImageLockMode.ReadWrite, d2.PixelFormat)
            Dim ptr2 As IntPtr = bmpData2.Scan0
            Dim bytes2 As Integer = Math.Abs(bmpData2.Stride) * d2.Height
            Dim rgbValues2(bytes2 - 1) As Byte
            Dim rgbValues3() As Byte = rgbValues2.Clone
            Dim pixelbyte2 As Integer = Math.Abs(bmpData2.Stride) \ bmpData2.Width 'should be 4
            System.Runtime.InteropServices.Marshal.Copy(ptr2, rgbValues2, 0, bytes2)
            For i = 0 To border2 - 1
                For y = 1 To d2.Height - 1
                    For x = 0 To d2.Width - 1
                        Dim a1, a2 As Integer
                        a1 = rgbValues2((y * d2.Width + x) * 4 + 3)
                        a2 = rgbValues2(((y - 1) * d2.Width + x) * 4 + 3)
                        If a1 + a2 = 255 Then
                            If a1 = 255 Then
                                rgbValues3(((y - 1) * d2.Width + x) * 4 + 3) = 255
                                rgbValues3(((y - 1) * d2.Width + x) * 4 + 2) = 255
                                rgbValues3(((y - 1) * d2.Width + x) * 4 + 1) = 255
                                rgbValues3(((y - 1) * d2.Width + x) * 4 + 0) = 255
                            Else
                                rgbValues3((y * d2.Width + x) * 4 + 3) = 255
                                rgbValues3((y * d2.Width + x) * 4 + 2) = 255
                                rgbValues3((y * d2.Width + x) * 4 + 1) = 255
                                rgbValues3((y * d2.Width + x) * 4 + 0) = 255
                            End If
                        End If
                    Next
                Next
                For y = 0 To d2.Height - 1
                    For x = 1 To d2.Width - 1
                        Dim a1, a2 As Integer
                        a1 = rgbValues2((y * d2.Width + x) * 4 + 3)
                        a2 = rgbValues2((y * d2.Width + x - 1) * 4 + 3)
                        If a1 + a2 = 255 Then
                            If a1 = 255 Then
                                rgbValues3((y * d2.Width + x - 1) * 4 + 3) = 255
                                rgbValues3((y * d2.Width + x - 1) * 4 + 2) = 255
                                rgbValues3((y * d2.Width + x - 1) * 4 + 1) = 255
                                rgbValues3((y * d2.Width + x - 1) * 4 + 0) = 255
                            Else
                                rgbValues3((y * d2.Width + x) * 4 + 3) = 255
                                rgbValues3((y * d2.Width + x) * 4 + 2) = 255
                                rgbValues3((y * d2.Width + x) * 4 + 1) = 255
                ...
                rgbValues2 = rgbValues3.Clone
            Next
            Dim element2 As Integer = -1
            While element2 < 0 Or element2 > 2
                element2 = CInt(Int(Val(InputBox("Element? [0=Cute, 1=Cool, 2=Passion]", "Direct Export", 0))))
            End While
            For y = 0 To bmpData2.Height - 1
                Dim r, g, b As Integer
                Select Case y
                    Case 0 To bmpData2.Height \ 3
                        r = {255, 125, 255}(element2)
                        g = {138, 215, 210}(element2)
                        b = {232, 255, 75}(element2)
                    Case bmpData2.Height \ 3 + 1 To bmpData2.Height * 2 \ 3
                        Dim p As Double = (y - (bmpData2.Height \ 3 + 1)) / ((bmpData2.Height * 2 \ 3) - (bmpData2.Height \ 3 + 1))
                        r = CInt({255, 125, 255}(element2) + {-148, 130, 0}(element2) * p)
                        g = CInt({138, 215, 210}(element2) + {95, 14, -72}(element2) * p)
                        b = CInt({232, 255, 75}(element2) + {23, -122, 157}(element2) * p)
                    Case bmpData2.Height * 2 \ 3 + 1 To bmpData2.Height - 1
                        r = {107, 255, 255}(element2)
                        g = {233, 229, 138}(element2)
                        b = {255, 133, 232}(element2)
                End Select
                Dim baseoffset As Integer = y * bmpData2.Width
                For x = 0 To bmpData2.Width - 1
                    If rgbValues2((baseoffset + x) * pixelbyte + 3) > 0 Then
                        'MsgBox(x & "," & y & ": " & rgbValues22((baseoffset + x) * pixelbyte + 3))
                        rgbValues2((baseoffset + x) * pixelbyte + 3) = 255
                        rgbValues2((baseoffset + x) * pixelbyte + 2) = r
                        rgbValues2((baseoffset + x) * pixelbyte + 1) = g
                        rgbValues2((baseoffset + x) * pixelbyte + 0) = b
                    End If
                Next
            Next
            System.Runtime.InteropServices.Marshal.Copy(rgbValues2, 0, ptr2, bytes2)
            d2.UnlockBits(bmpData2)
            Dim completed2 As New Bitmap(d.Width, d.Height)
            Using g As Graphics = Graphics.FromImage(completed2)
                g.DrawImage(d, New Rectangle(New Point(0, 0), d.Size))
                g.DrawImage(d2, New Rectangle(New Point(0, 0), d.Size))
            End Using
            SaveFileDialog1.FileName = System.IO.Path.GetFileNameWithoutExtension(OpenFileDialog2.FileName) & "_sign.png"
            If SaveFileDialog1.ShowDialog = DialogResult.OK Then
                completed2.Save(SaveFileDialog1.FileName)
            End If
        End If
    End Sub
End Class
